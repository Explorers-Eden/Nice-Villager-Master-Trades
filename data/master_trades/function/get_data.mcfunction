tag @n[type=item,predicate=master_trades:entity/item_count_1,distance=..0.75] add trade.item
...
execute as @n[type=item,tag=trade.item] if items entity @s container.0 #master_trades:low_value_trades store result storage eden:temp master_trades.price int 1 run random value 1..12
execute as @n[type=item,tag=trade.item] if items entity @s container.0 #master_trades:mid_value_trades store result storage eden:temp master_trades.price int 1 run random value 13..28
execute as @n[type=item,tag=trade.item] if items entity @s container.0 #master_trades:high_value_trades store result storage eden:temp master_trades.price int 1 run random value 29..48

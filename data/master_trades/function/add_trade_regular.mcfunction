...
$execute if items entity @n[type=item,tag=trade.item] container.0 #master_trades:single_item_trades run \
...
$execute unless items entity @n[type=item,tag=trade.item] container.0 #master_trades:single_item_trades unless items entity @n[type=item,tag=trade.item] container.0 minecraft:enchanted_book run \
data modify entity @s Offers.Recipes append value \
{maxUses:$(uses),buy:{id:"minecraft:emerald",count:$(price)},sell:{id:"$(id)",count:$(count)},xp:20,priceMultiplier: 0.1f, demand: -24}

tag @s add master_trades.master.trade.set
